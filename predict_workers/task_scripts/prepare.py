...
class PrepareWorker(BaseWorker):
    ...
    def perform_task(self, task, db_session):
        ...
        self.update_task_status(
            db_session, task, PredictionState.PREPARING, workdir=workdir
        )
        ...
        if workdir.exists():
            self.log.warning(
                f"Task directory {workdir} already exists. Removing it for re-run task."
            )
        ...
        json_data = {
            ...
            "predict_params": {
                ...
                "maximum_visual_magnitude": int(
                    task.input_manifest.get("maximum_visual_magnitude", 18)
                ),
                "ephemeris_step": int(task.input_manifest.get("ephemeris_step", 60)),
                "asteroid_ephemeris": self.get_asteroid_ephemeris_info(
                    asteroid.get("name")
                ),
                "star_catalog": self.get_star_catalog_params(
                    task.input_manifest.get("star_catalog", "gaia_dr3")
                ),
                "planetary_ephemeris": self.get_planetary_ephemeris_params(
                    task.input_manifest.get("planetary_ephemeris", "de440")
                ),
                "leap_seconds": self.get_leap_seconds_params(
                    task.input_manifest.get("leap_seconds", "naif0012")
                ),
            },
            "asteroid": asteroid,
        }
