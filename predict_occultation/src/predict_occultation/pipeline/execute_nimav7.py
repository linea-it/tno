...
def NIMAmanager(inputParametersFile):
    ...
    if os.path.exists(parameters[3] + "/" + name + ".rwo") and os.path.exists(
        parameters[4] + "/" + name + ".eq0"
    ):
        pass
    elif os.path.exists(parameters[3] + "/" + name + ".rwm") and os.path.exists(
        parameters[4] + "/" + name + ".eqm"
    ):
        pass
    else:
        print(f"There are no files of the {name}")
        exit(0)
    ...
    subprocess.call(
        ["cp", parameters[2] + "/" + name + ".bsp", pathNIMAuser + "/jplbsp"]
    )
    ...
    executeScript(
        "./sc_AstDySMPC2NIMA.sh", np.concatenate([parameters[5:7], parameters[3:5]])
    )
    ...
    executeScript(
        "./sc_importbsp.sh", np.append(parameters[5:7], "jplbsp/" + name + ".bsp")
    )
