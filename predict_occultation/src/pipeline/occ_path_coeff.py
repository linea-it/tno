...
# Enable with: RESOURCE_MONITOR=1
from pipeline.resource_monitor import get_resource_monitor
...
def safe_moon_fraction(
    date_str: str, source_id: int, benchmark=None
) -> Union[float, None]:
    ...
    try:
        ...
        # BENCHMARK: START
        if benchmark:
            benchmark.record_timeout("moon_illuminated_fraction")
        # BENCHMARK: END
        print(f"INFO: Skipped moon fraction for gaia_source_id {source_id} (timeout).")
    except Exception as e:
        # BENCHMARK: START
        if benchmark:
            benchmark.record_error("moon_illuminated_fraction")
        # BENCHMARK: END
        print(f"ERROR: Moon fraction calculation failed for {source_id}: {e}")
    return None


def safe_occ_path_coeff(row: pd.Series, benchmark=None) -> dict:
    ...
    try:
        ...
        # BENCHMARK: START
        if benchmark:
            benchmark.record_timeout("path_coefficients")
        # BENCHMARK: END
        print(
        ...
        # BENCHMARK: START
        if benchmark:
            benchmark.record_error("path_coefficients")
        # BENCHMARK: END
...
def run_occultation_path_coeff(
    ...
    try:
        ...
        # -------------------------------------------------
        # BENCHMARK: START
        with bm.section("spice_kernel_loading"):
            # BENCHMARK: END
            # RESOURCE_MONITOR: START
            rm.section_start("spice_kernel_loading")
            # RESOURCE_MONITOR: END
            loaded_kernels = {
                spice.kdata(i, "ALL")[0] for i in range(spice.ktotal("ALL"))
            }
            kernels_to_load = {
            ...
            # RESOURCE_MONITOR: START
            rm.section_end("spice_kernel_loading")
            # RESOURCE_MONITOR: END
        ...
        # -------------------------------------------------
        # BENCHMARK: START
        with bm.section("file_reading_predictions"):
            # BENCHMARK: END
            # RESOURCE_MONITOR: START
            rm.section_start("file_reading")
            # RESOURCE_MONITOR: END
        ...
        # BENCHMARK: START
        with bm.section("file_reading_gaia_catalog"):
            # BENCHMARK: END
            gaia_catalog_path = os.path.join(
                obj_data.get("path", "."), "gaia_catalog.csv"
            )
        ...
        # BENCHMARK: START
        with bm.section("file_reading_uncertainties"):
            # BENCHMARK: END
        ...
        # BENCHMARK: START
        num_rows = len(df)
        with bm.section("coordinate_conversions", row_count=num_rows):
            # BENCHMARK: END
        ...
        # -------------------------------------------------
        # BENCHMARK: START
        with bm.section("star_matching_kdtree", row_count=len(df)):
            # BENCHMARK: END
        ...
        # -------------------------------------------------
        # BENCHMARK: START
        with bm.section("julian_date_conversion", row_count=len(df)):
            # BENCHMARK: END
            julian_dates = to_julian_date(df["date_time_obj"])

        # BENCHMARK: START
        with bm.section("apparent_diameter_duration", row_count=len(df)):
            # BENCHMARK: END
        ...
        # BENCHMARK: START
        with bm.section("apparent_magnitude", row_count=len(df)):
            # BENCHMARK: END
            # These library functions are not vectorized, so we use .apply
            if mag_cs:
                df["apparent_magnitude"] = pd.Series(
                    mag_cs(julian_dates), index=df.index
                )
            else:
        ...
        # BENCHMARK: START
        with bm.section("magnitude_drop", row_count=len(df)):
            # BENCHMARK: END
        ...
        # BENCHMARK: START
        with bm.section("moon_sun_separation", row_count=len(df)):
            # BENCHMARK: END
        ...
        # BENCHMARK: START
        with bm.section("moon_illuminated_fraction", row_count=len(df)):
            # BENCHMARK: END
            df["moon_illuminated_fraction"] = df.apply(
                lambda r: safe_moon_fraction(
                    r["date_time"], r["gaia_source_id"], benchmark=bm
                ),
                axis=1,
            )
        ...
        # BENCHMARK: START
        with bm.section("uncertainty_calculations", row_count=len(df)):
            # BENCHMARK: END
            if has_uncertainties:
                ...
                def apply_uncertainty_calcs(row):
                    ...
                    return pd.Series(
                        [inst, close], index=["instant", "closest_approach"]
                    )
                ...
                rad_angle = (
                    np.deg2rad(
                        pd.to_numeric(
                            df["closest_approach_uncertainty"], errors="coerce"
                        )
                        / 3600
                    )
                    / 2
        ...
        # BENCHMARK: START
        with bm.section("path_coefficients", row_count=len(df)):
            # BENCHMARK: END
            # RESOURCE_MONITOR: START
            rm.section_end("file_reading")
            rm.section_start("path_coefficients")
            # RESOURCE_MONITOR: END
            path_coeffs_results = df.apply(
                lambda r: safe_occ_path_coeff(r, benchmark=bm), axis=1
            )
            # RESOURCE_MONITOR: START
            rm.section_end("path_coefficients", row_count=len(df))
            # RESOURCE_MONITOR: END
        ...
        # BENCHMARK: START
        with bm.section("hash_generation", row_count=len(df)):
            # BENCHMARK: END
            df["hash_id"] = df.apply(
                lambda r: (
                    generate_hash(
        ...
        # BENCHMARK: START - delete this block to remove benchmarking
        try:
            total_rows = len(df) if "df" in locals() and df is not None else None
            bm.save(total_rows=total_rows)
        except:
            pass  # Don't fail if benchmark save fails
        # BENCHMARK: END
        # RESOURCE_MONITOR: START - delete this block to remove resource monitoring
        try:
            rm.save()
        except:
            pass  # Don't fail if resource monitor save fails
        # RESOURCE_MONITOR: END
