...
def run_occultation_path_coeff(
    ...
    try:
        with profiler("SPICE Kernel Loading"):
            loaded_kernels = {
        ...
        with profiler("Read Predict Table CSV"):
            df = pd.read_csv(
                predict_table_path,
                delimiter=";",
                header=None,
        ...
        with profiler("Read Gaia Catalog CSV"):
        ...
        if mag_and_uncert_path.exists():
            with profiler("Read & Process Uncertainties JSON"):
        ...
        with profiler("Data Cleaning & Type Conversion"):
            df.rename(
                columns={
            ...
            df.dropna(subset=["date_time_obj"], inplace=True)

        with profiler("Star Matching (cKDTree)"):
        ...
        with profiler("Primary Calculations (Vectorized)"):
        ...
        with profiler("Calc: Apparent Magnitude"):
            if mag_cs:
        ...
        with profiler("Calc: Magnitude Drop"):
            df["magnitude_drop"] = compute_magnitude_drop(
                df["apparent_magnitude"], df["g_star"]
            )

        with profiler("Calc: Moon/Sun Separation"):
        ...
        with profiler("Calc: Moon Illuminated Fraction (apply)"):
            df["moon_illuminated_fraction"] = df.apply(
                lambda r: safe_moon_fraction(r["date_time"], r["gaia_source_id"]),
                axis=1,
            )

        with profiler("Calc: All Uncertainties"):
        ...
        with profiler("Add Asteroid & Provenance Data"):
            ast_data_cols = [
                "name",
                "number",
                "base_dynclass",
        ...
        with profiler("Finalization and Output"):
        ...
        with profiler("SPICE Kernel Clear"):
            spice.kclear()
