...
def ascii_to_csv(inputFile, outputFile):
    """
    Function to convert data from an ASCII table (generated by PRAIA OCC) to a CSV file.
    ...
    # Extracting positions of stars and objects and save it in an array
    for i in range(6, 17, 3):
        dateAndPositions.append(get_position_from_occ_table(data, [i, i + 1, i + 2]))

    # Use dtype=object to keep datetime objects and strings in the same array
    dateAndPositions = np.array(dateAndPositions, dtype=object).T

    # Extracting other parameters (C/A, P/A, etc.)
