...
- Add daytime layer to the map
- Add download functionality to maps and kmz
- **backend/newsletter/process_event_filter.py**: Add new white transparent linea logo
- **backend/tno/tasks.py**: Implements pre-caching task for asteroid classes
- **backend/coreAdmin/celery.py**: Implements update asteroid class cache celery cronjob
- Updates get_occultation_paths endpoint with new data and remove unnecessary values
- **backend/newsletter/process_event_filter.py**: Add new white transparent linea logo
- **backend/tno/tasks.py**: Implements pre-caching task for asteroid classes
- **backend/coreAdmin/celery.py**: Implements update asteroid class cache celery cronjob
...
- **tasks.py**: Fix bug in update asteroid classes cache
- **library.py,occ_path_coeff.py**: Fix h > 99 bug and expands hash
- **backend/coreAdmin/celery.py**: Add minute parameter to task schedule on update_asteroid_classes_cache
- **Redirects-page-to-newsletter-settings-on-login**: Updates to  frontend/src/services/api/Auth.js and frontend/src/pages/PublicPortal/Login/index.js were done to ensure proper redirection
- **tasks.py**: Fix bug in update asteroid classes cache
- **library.py,occ_path_coeff.py**: Fix h > 99 bug and expands hash
- **backend/coreAdmin/celery.py**: Add minute parameter to task schedule on update_asteroid_classes_cache
- **Redirects-page-to-newsletter-settings-on-login**: Updates to  frontend/src/services/api/Auth.js and frontend/src/pages/PublicPortal/Login/index.js were done to ensure proper redirection
- Fixes to merge with main
- Fixes introduced bug in ClosestApproachUncertaintyField component
- **backend/coreAdmin/celery.py**: Bug fix in caching celery task
- **backend/newsletter/process_event_filter.py**: Fix time comparison into full day
- **backend/coreAdmin/settings.py**: Update noreply address
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/index.js**: Fixes bugs and implements enhaced version of dinamic maps
- **backend/tno/occviz.py**: Fix bug returning body limits to endpoint
- react-leaflet downgrade to version 4
- Missing packages installed
- **backend/newsletter/process_event_filter.py**: Fix time comparison into full day
- **backend/coreAdmin/settings.py**: Update noreply address
- Fixes in pre-commit
- **backend/newsletter/process_event_filter.py**: Fix bug in processing logic
- Fix templates to work properly on gmail
- **compose/testing/docker-compose-template.yml**: Corrige a impossibilidade de downloads dos csvs
- **backend/newsletter/process_event_filter.py**: Corrige bug na logica da execução da subscrição
- Fix heading and linea logo
- Fix pre commit bug
- Fix bug asteroid name select event filter form
- **frontend/src/pages/PublicPortal/Newsletter/UserEventFilters.js**: Fix frequency exhibition
...
- **frontend/src/pages/AsteroidJob/AsteroidJobDetail.js**: Mudança no componente e layout da pagina de  detalhes asteroid_job
- **frontend/src/pages/AsteroidJob/AsteroidJobDetail.js**: Correção na consulta a api no componente da pagina de detalhes de job dos asteroides
- Optimizes how buttons are exhibited in mobile screens
- Implements thumbnails creation and refactors how SORA maps are generated
- **frontend/src/components/OccultationMap/OccultationKmzDownload.js**: Change colors in kmz file
- Optimize map exibition
- **frontend/src/components/OccultationMap/Legend.js**: centralização da legenda dinamica para exibição em telas pequenas
- **frontend/src/components/OccultationMap/OccultationKmzDownload.js**: correção do nome das variaveis que contem as coordenadas dos pontos
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/index.js**: Inclusao dos pararmentros mapCenter e diameter na chamada do componente que gera o kmz
- **frontend/src/components/OccultationMap/OccultationKmzDownload.js**: Inclusao da função que calcula as coordenadas e escreve no kml os pontos menores
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/index.js**: Inclusão do parametro com as coordenadas dos pontos menores
- **frontend/src/components/OccultationMap/OccultationKmzDownload.js**: Otimização do código que escreve o kml
- **frontend/src/components/OccultationMap/OccultationKmzDownload.js-frontend/src/pages/PredictionEvents/partials/OccultationMap/index.js**: Alteração na passagem dos parametros para o componente que gera o arquivo kmz, substitui os paramentros escritos um a um pelo operador spread
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/index.js**: Inclusão da chamada do componente que gera o arquivo kmz dos mapas
- **frontend/src/components/OccultationMap/OccultationKmzDownload.js**: Implementação do componente que gera o arquivo kmz dos mapas
- **frontend/package.json-frontend/yarn.lock**: Instalação das bibliotecas file-saver e jszip
- **backend/newsletter/events_send_mail.py**: Implements formating logic to send emails
- **backend/newsletter/templates/results.html**: Removes formatting logic from template
- **frontend/src/pages/PredictionEvents/Detail.js**: Correção no layout dos cards da pagina de detalhes
- **frontend/src/components/Subscription/index.js**: Correção na responsividade do campo de email da subscrição
- **frontend/src/components/OccultationMap/OccultationMapDownload.js**: Correção do style do botao de download do mapa
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/index.js**: Correção no layout do botao para download do mapa
- **frontend/src/components/OccultationMap/OccultationMapDownload.js**: Correção no nome do botao de download
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/index.js**: Inclusão da chamada do componente que faz o download do mapa no formato do Sora
- **frontend/src/components/OccultationMap/OccultationMapDownload.js**: Componente que faz o download do mapa no formato do Sora
- **backend/newsletter/events_send_mail.py**: Implements formating logic to send emails
- **backend/newsletter/templates/results.html**: Removes formatting logic from template
- **frontend/src/components/Subscription/index.js**: Correção na responsividade do campo de email da subscrição
- **frontend/src/components/OccultationMap/NightTime.js**: Correção no componente que desenhas as sombras para evitar a dupla plotagem
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/index.js**: Limpeza do codigo
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/FlyToMap.js-frontend/src/pages/PredictionEvents/partials/OccultationMap/Legend.js-frontend/src/pages/PredictionEvents/partials/OccultationMap/NightTime.js-frontend/src/pages/PredictionEvents/partials/OccultationMap/index.js-frontend/src/components/OccultationMap/**: Os componentes utiizados no mapa foram movidos da pasta PredictionEvents/Occultation para uma pasta especifica em componentes (componentes/Occultation)
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/FlyToMap.js**: Separação da função FlyToMap em um componente unico
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/index.js**: Separação do componente FlyToMap que move o mapa para a posição especificada com zoom
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/Legend.js**: Separação da função que desenha as lelgendas em um componente unico
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/NightTime.js**: Correção do componente que desenha as sombras incluindo o return null
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/index.js**: Separação do componente Legend que desenha as legendas dinamicas
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/NightTime.js**: Retirada do tyle com as luzes das cidades. Agora plota somente a sombra uma unica vez
- **backend/tno/views/occultation.py**: Fixes pre-commit
- Remoção de arquivos desnecessarios
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/index.js**: Inclusao do componente que desenha as sombras no plot do mapa
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/NightTime.js**: Criação do componente que desenha as sombras noite e dia
- **frontend/package.json-frontend/yarn.lock**: Instalação dos plugins leaflet-terminator e leaflet-boundary-canvas
- **backend/newsletter/templates/results.html**: Refactors the email for clarity
- **.pre-commit-config.yaml**: Comments eslint hooks on pre-commit
- Updates the templates with new logo and button
- **backend/newsletter/events_send_mail.py-backend/newsletter/process_event_filter.py**: Limpeza do código
- **backend/newsletter/events_send_mail.py**: Verificação dos usuarios que estao com subscrição ativa e inclusao do if para enviar os emails somente para que está inscrito
- **frontend/src/pages/PublicPortal/Newsletter/DeleteAccount.js**: Exclusao do alert de sucesso após exclusao da conta. Não será mais necessário
- **backend/newsletter/templates/results.html**: Inclusao do link para a documentação do portal no template html
- **frontend/src/pages/PublicPortal/Newsletter/DeleteAccount.js**: Inclusao de um snackbar e um alert informando que a conta do usuario foi deletada com sucesso
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/index.js**: Retirada dos parametros para os thumbnail, não serão necessários
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/index.js**: Remoção do import da biblioteca do leaflet. Ela está sendo importada no app.js
- **.pre-commit-config.yaml**: Comments eslint hooks
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/leaflet.css**: Arquivo movido para pasta assets
- **frontend/src/App.js**: Inclusão do import do css da leaflet
- **frontend/src/assets/css/leaflet.css**: Mudança do css do leaflet para a pasta correta
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/index.js**: Inclusao de verificação das variáveis antes dos plots
- **package.json**: Exclusao de arquivo desnecessario
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/index.js**: Correção do bug na plotagem do mapa e refatoração do codigo
- Updates the templates with new logo and button
- **backend/newsletter/events_send_mail.py-backend/newsletter/process_event_filter.py**: Limpeza do código
- **backend/newsletter/events_send_mail.py**: Verificação dos usuarios que estao com subscrição ativa e inclusao do if para enviar os emails somente para que está inscrito
- **frontend/src/pages/PublicPortal/Newsletter/DeleteAccount.js**: Exclusao do alert de sucesso após exclusao da conta. Não será mais necessário
- **backend/newsletter/templates/results.html**: Inclusao do link para a documentação do portal no template html
- **frontend/src/pages/PublicPortal/Newsletter/DeleteAccount.js**: Inclusao de um snackbar e um alert informando que a conta do usuario foi deletada com sucesso
- **frontend/src/pages/PredictionEvents/partials/OccultationMap/index.js**: Refatoração e limpeza do componente de mapas
- **frontend/src/services/api/Occultation.js**: Inclusao da consult ao endpoint get_occultation_paths
- **package.json-yarn.lock**: Instalação das bibliotecas leaflet e react-leaflet pra plotagem de mapas
- **backend/newsletter/templates/results.html**: Refactor email template with results
- **Refatora-email-com-resultados**: Melhora o estilo e adiciona mais informações importantes no email
- **frontend/src/pages/PublicPortal/Login/index.js**: Refactor login page
- Backend related email templates refactor and minor bug fixes
- Complete refactoring of email templates
