...
def download_ccd(idx, ccd, base_url, ccd_image_dir, auth):
    ...
    try:
        ...
        download_url = "/".join([base_url.strip("/"), ccd["path"].strip("/"), filename])
...
def download_des_ccds(job_id, ccds, max_workers=10):
    ...
    tdelta = finish - start
    logger.debug(
        "Downloaded [%s] CCDs In %s." % (downloaded, humanize.naturaldelta(tdelta))
    )

    if failed > 0:
        logger.warning("%s CCDs have failed and have not been downloaded." % failed)
...
def plot_time_profile(filepath, job_path):
    ...
    df = pd.read_csv(filepath, delimiter=";", usecols=["start_time", "finish_time"])
    ...
    ax.xaxis_date()
    plt.hlines(df.index, dt.date2num(df.start_time), dt.date2num(df.finish_time))
    plt.xticks([], [])
...
def estimate_time_and_size(to_execute):
    ...
    return dict({"estimated_time": estimated_time, "estimated_size": estimated_size})
...
def run_job(job_id):
    ...
    try:
        ...
        if os.path.exists(jobpath) and settings.DEBUG is True:
            logger.info("Job directory already exists. JobPath: [%s]" % jobpath)
            try:
        ...
        ccds = spdao.ccds_for_position(
            start=dt_start, end=dt_end, dynclass=job["dynclass"], name=job["name"]
        )
        ...
        if job["name"] is not None:
            ...
            t_asteroids = spdao.count_asteroids_by_dynclass(
                start=dt_start, end=dt_end, dynclass=job["dynclass"]
            )
        ...
        if job["ccds_to_download"] == ccds_downloaded:
            ...
            job["error"] = (
                "%s CCDs have not been downloaded, please try again." % not_downloaded
            )
        ...
        db.complete_job(job)
        logger.info(
            "Job completed %s" % humanize.naturaldelta(tdelta, minimum_unit="seconds")
        )
...
def start_pipeline():
    ...
    if len(running) == 0:
        ...
        if len(idle) > 0:
            ...
            logger.debug("There are %s jobs waiting to run" % len(idle))

            to_run = idle[0]

            run_job(to_run["id"])
        else:
            logger.debug("No jobs in the queue to run.")
    else:
        logger.debug("There is already a job running.")
