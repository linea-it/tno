...
class DesSkybotJobResultDao(DBBase):
    ...
    def import_data(self, dataframe):
        ...
        try:
            ...
            sql = (
                "COPY %s (ticket, success, error, execution_time, positions, inside_ccd, outside_ccd, filename, exposure_id, job_id, ccds_with_asteroids) FROM STDIN with (FORMAT CSV, DELIMITER '|', HEADER);"
                % table
            )
    ...
    def count_exec_by_period(self, start, end):
        ...
        stm_join = de_tbl.join(
            ds_tbl, de_tbl.c.id == ds_tbl.c.exposure_id, isouter=True
        )
        ...
        stm = (
            ...
            .where(
                and_(
                    ...
                    de_tbl.c.id.notin_(select([ds_tbl.c.exposure_id])),
                )
            )
    ...
    def count_not_exec_by_period(self, start, end):
        ...
        stm = select([func.count(de_tbl.c.id).label("total")]).where(
            and_(
                de_tbl.c.date_obs.between(str(start), str(end)),
                de_tbl.c.id.notin_(select([ds_tbl.c.exposure_id])),
            )
    ...
    def t_positions_des_by_job(self, job_id):
        ...
        stm = (
            select([func.count(sp.c.id).label("count")])
    ...
    def t_unique_objects_by_job(self, job_id):
        ...
        stm = (
            select([func.count(sp.c.name.distinct()).label("count")])
            .select_from(sp.join(ds, sp.c.ticket == ds.c.ticket))
            .where(and_(ds.c.job_id == int(job_id)))
        )
    ...
    def t_exposures_with_objects_by_job(self, job_id):
        ...
        stm = (
            select([func.count(dsp.c.exposure_id.distinct()).label("count")])
    ...
    def t_ccds_with_objects_by_job(self, job_id):
        ...
        stm = (
            select([func.count(dsp.c.ccd_id.distinct()).label("count")])
            .select_from(dsp.join(ds, dsp.c.ticket == ds.c.ticket))
            .where(and_(ds.c.job_id == int(job_id)))
        )
    ...
    def dynclass_asteroids_by_job(self, job_id):
        ...
        stm = text(
            """select split_part(dynclass, '>', 1) as dynclass, count(distinct(sp.name)) as asteroids from skybot_position sp inner join des_skybotposition dsp on	sp.id = dsp.position_id inner join des_skybotjobresult ds on sp.ticket = ds.ticket where ds.job_id = %s group by split_part(dynclass, '>', 1) order by split_part(dynclass, '>', 1);"""
            % int(job_id)
        )
    ...
    def dynclass_ccds_by_job(self, job_id):
        ...
        stm = text(
            """select split_part(dynclass, '>', 1) as dynclass, count(distinct(dsp.ccd_id)) as ccds from skybot_position sp inner join des_skybotjobresult ds on sp.ticket = ds.ticket inner join des_skybotposition dsp on sp.id = dsp.position_id where ds.job_id = %s group by split_part(dynclass, '>', 1) order by split_part(dynclass, '>', 1);"""
            % int(job_id)
        )
    ...
    def dynclass_positions_by_job(self, job_id):
        ...
        stm = text(
            """select split_part(dynclass, '>', 1) as dynclass, count(dsp.id) as positions from skybot_position sp inner join des_skybotjobresult ds on sp.ticket = ds.ticket inner join des_skybotposition dsp on dsp.position_id = sp.id where ds.job_id = %s group by split_part(dynclass, '>', 1) order by split_part(dynclass, '>', 1);"""
            % int(job_id)
        )
    ...
    def dynclass_band_by_job(self, job_id, dynclass):
        ...
        stm = text(
            "select de.band, count(sp.id) as positions from des_exposure de inner join des_skybotposition dsp on dsp.exposure_id = de.id inner join des_skybotjobresult ds on ds.exposure_id = dsp.exposure_id inner join skybot_position sp on dsp.position_id = sp.id where ds.job_id = %s and split_part(dynclass, '>', 1) = '%s' group by de.band;"
            % (int(job_id), dynclass)
        )
    ...
    def dynclass_asteroids_by_id(self, id):
        ...
        stm = text(
            "select split_part(sp.dynclass, '>', 1) as dynclass, count(distinct(sp.name)) as asteroids, count(distinct(dsp.ccd_id)) as ccds from des_skybotposition dsp inner join des_skybotjobresult ds on ds.exposure_id = dsp.exposure_id inner join skybot_position sp on sp.id = dsp.position_id where ds.id = %s group by split_part(dynclass, '>', 1) order by split_part(dynclass, '>', 1);"
            % int(id)
        )
    ...
    def count_nights_analyzed_by_period(self, start, end):
        ...
        stm = (
            select([func.count(cast(de_tbl.c.date_obs, Date).distinct())])
            .select_from(
                ds_tbl.join(de_tbl, de_tbl.c.id == ds_tbl.c.exposure_id, isouter=True)
            )
            .where(and_(de_tbl.c.date_obs.between(str(start), str(end))))
        )
    ...
    def count_ccds_analyzed_by_period(self, start, end):
        ...
        stm = (
            select([func.count(dc_tbl.c.id)])
            .select_from(
                ds_tbl.join(
                    de_tbl, de_tbl.c.id == ds_tbl.c.exposure_id, isouter=True
                ).join(
                    dc_tbl, dc_tbl.c.exposure_id == ds_tbl.c.exposure_id, isouter=True
                )
            )
            .where(and_(de_tbl.c.date_obs.between(str(start), str(end))))
        )
