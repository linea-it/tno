...
class DESImportSkybotPositions(ImportSkybotPositions):
    ...
    def import_des_skybot_positions_q3c(self, exposure_id, ticket, filepath):
        ...
        try:
            ...
            if total_position == 0:
                ...
                for ccd in ccds:
                    try:
                        ...
                        self.logger.debug(
                            ...
                            % (
                                ...
                                humanize.naturaldelta(
                                    tdelta, minimum_unit="milliseconds"
                                ),
                            )
                        )
                        ...
                        msg = (
                            "Failed to make ccd association CCD ID: [%s] CCD Num: [%s] Error: %s"
                            % (ccd["id"], ccd["ccdnum"], e)
                        )
                        raise Exception(msg)
                ...
                self.logger.info(
                    "Exposure: [%s] Positions [ %s ] Inside CCDs: [ %s ] Outside: [ %s ] in %s"
                    % (
                        exposure_id,
                        str(total_position),
            ...
            self.logger.debug(
                "Completed data import for exposure [%s] in %s"
                % (
                    exposure_id,
                    humanize.naturaldelta(a_tdelta, minimum_unit="milliseconds"),
                )
            )
    ...
    def ccds_by_exposure_id(self, exposure_id):
        ...
        try:
            ...
            raise Exception(
                "Failed to retrieve CCDs for exposure with exposure_id=%s. Error: [%s]"
                % (exposure_id, e)
            )
    ...
    def import_des_skybot_positions(self, exposure_id, ticket, filepath, job_id):
        ...
        try:
            ...
            if total_position == 0:
                ...
                self.logger.debug("CCDS With Asteroids: %s" % ccds_with_asteroids)
                ...
                if total_inside_ccd > 0:
                    ...
                    df_inside.drop(columns=["raj2000", "decj2000"], inplace=True)
                    # Renomeia a coluna id para position_id
                    df_inside.rename(columns={"id": "position_id"}, inplace=True)
                    ...
                    total_imported = self.import_positions_inside_ccd(df_inside)
            ...
            result.update({"success": False, "error": str(e), "traceback": trace})
            self.logger.error(trace)
            self.logger.error(e)
    ...
    def import_positions_inside_ccd(self, dataframe):
        ...
        try:
            ...
            sql = (
                "COPY %s (ccd_id, exposure_id, position_id, ticket) FROM STDIN with (FORMAT CSV, DELIMITER '|', HEADER);"
                % table
            )
