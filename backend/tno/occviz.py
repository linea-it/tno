...
def visibility(latitude, longitude, radius, date_time, star_coordinates, closest_approach, position_angle, velocity, delta_distance, offset=[0, 0], object_radius=None, ring_radius=None, ignore_nighttime=False, latitudinal=False, interpolate=True):
    '''
...
def occultation_path_coeff2(
    ...
    output = {
        ...
        't1': None,
        'coeff_latitude': None,
        'coeff_longitude': None,
        'body_upper_coeff_latitude': None,
        'body_upper_coeff_longitude': None,
        'body_lower_coeff_latitude': None,
        'body_lower_coeff_longitude': None,
        'ring_upper_coeff_latitude': None,
        'ring_upper_coeff_longitude': None,
        'ring_lower_coeff_latitude': None,
        'ring_lower_coeff_longitude': None,
        'min_longitude': None,
        'max_longitude': None
    }
    ...
    output.update({
        'coeff_latitude': list(result[1]) if isinstance(result[1], np.ndarray) else None,
        'coeff_longitude': list(result[0]) if isinstance(result[1], np.ndarray) else None
    })
    lons.append([result[2], result[3]])
    ...
    if object_radius is not None:
        upper_limit = _path_latlon_coeff(
            instants, dtimes, centers, delta, ca, vel, pa, pa_plus, radius=object_radius, degree=degree)
        output.update({
            'body_upper_coeff_latitude': list(upper_limit[1]) if isinstance(upper_limit[1], np.ndarray) else None,
            'body_upper_coeff_longitude': list(upper_limit[0]) if isinstance(upper_limit[0], np.ndarray) else None
        })
        lower_limit = _path_latlon_coeff(
            instants, dtimes, centers, delta, ca, vel, pa, pa_plus, radius=-object_radius, degree=degree)
        output.update({
            'body_lower_coeff_latitude': list(lower_limit[1]) if isinstance(lower_limit[1], np.ndarray) else None,
            'body_lower_coeff_longitude': list(lower_limit[0]) if isinstance(lower_limit[0], np.ndarray) else None
        })
        lons.append([upper_limit[2], upper_limit[3],
                    lower_limit[2], lower_limit[3]])
    ...
    if ring_radius is not None:
        ring_upper_limit = _path_latlon_coeff(
            instants, dtimes, centers, delta, ca, vel, pa, pa_plus, radius=ring_radius, degree=degree)
        output.update({
            'ring_upper_coeff_latitude': list(ring_upper_limit[1]) if isinstance(ring_upper_limit[1], np.ndarray) else None,
            'ring_upper_coeff_longitude': list(ring_upper_limit[0]) if isinstance(ring_upper_limit[0], np.ndarray) else None
        })
        ring_lower_limit = _path_latlon_coeff(
            instants, dtimes, centers, delta, ca, vel, pa, pa_plus, radius=-ring_radius, degree=degree)
        output.update({
            'ring_lower_coeff_latitude': list(ring_lower_limit[1]) if isinstance(ring_lower_limit[1], np.ndarray) else None,
            'ring_lower_coeff_longitude': list(ring_lower_limit[0]) if isinstance(ring_lower_limit[0], np.ndarray) else None
        })
        lons.append([ring_upper_limit[2], ring_upper_limit[3],
                    ring_lower_limit[2], ring_lower_limit[3]])
    ...
    try:
        output.update({
            'min_longitude': float(longitudes.min()),
            'max_longitude': float(longitudes.max())
        })
    except:
        output.update({
            'min_longitude': None,
