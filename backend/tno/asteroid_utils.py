...
def plot_observations_by_asteroid(name, format, width=600, height=400):
    ...
    eph_filename = f"{name}-%s-%s" % (date_start.strftime("%Y-%m-%d"), date_end.strftime("%Y-%m-%d"))
    ...
    if eph_filepath.exists():
        ...
        df_jpl.rename(columns={'datetime_str':'date', 'RA':'ra', 'DEC':'dec'}, inplace=True)
    ...
    fig1 = px.line(df_jpl, x=xDataJPL, y='dec', color_discrete_sequence=["black"], hover_data=["ra", "dec"])
    ...
    if 'correction_ra' in dfObs:
        ...
        textRA_DES='<b>ra</b>=%{customdata[0]:.3f}<br>' +\
                '<b>dec</b>=%{y:.3f}<br>' +\
                '<b>date</b>=%{customdata[1]}<br>'+\
                '<b>offset_ra</b>=%{customdata[2]:.3f}<br>'+\
                '<b>offset_dec</b>=%{customdata[3]:.3f}'
                
    fig2 = px.scatter(dfObs, x=xDataDES, y='dec', color_discrete_sequence=["red"], hover_data=["ra", "dec", "date_obs", "offset_ra", "offset_dec"])
    fig2.update_traces(marker=dict(size=20, symbol="square-open-dot"), name="DES", hovertemplate=textRA_DES)
    ...
    if 'correction_ra' in df_jpl: 
        ...
        full_fig = fig.full_figure_for_development(warn=False)  # recover data from figure
        ...
        tick_vals = [int(range_vl[0]) + distance_tick*num for num in range(number_ticks)] 
    ...
    plot_filename = f"plot_des_observations_{name}-%s-%s.{format}" % (date_start.strftime("%Y-%m-%d"), date_end.strftime("%Y-%m-%d"))
