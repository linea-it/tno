...
class ShibbolethBackend(object):
    ...
    def ensure_user(self, session_data):
        try:
            ...
            self.logger.debug(
                "Username: [ %s ] User Email: [ %s ]"
                % (
                    session_data.get("Shib-inetOrgPerson-cn"),
                    session_data.get("Shib-inetOrgPerson-mail"),
                )
            )

            user = User.objects.get(
                username=session_data.get("Shib-inetOrgPerson-cn"),
    ...
    def read_session_file(self, session_id):
        ...
        """
        filepath_crypt = os.path.join(settings.AUTH_SHIB_SESSIONS, session_id + ".aes")
        ...
        try:
            ...
            pyAesCrypt.decryptFile(filepath_crypt, filepath, password, bufferSize)
