...
def register_input(run_id, name, asteroid_input):
    ...
    asteroid = AstrometryAsteroid.objects.get(astrometry_run=run_id, name=name)
...
def update_job_status(job, condor_job):
    ...
    logger.info(
        "Update Condor Job ClusterId [ %s ] ProcId [ %s ] Status: [ %s ] "
...
def register_astrometry_outputs(astrometry_run, asteroid):
    logger = logging.getLogger("astrometry")
    logger.debug(
        "Register outputs for Astrometry Run [ %s ] Asteroid [ %s ]"
    ...
    try:
        ...
        if not os.path.exists(result_json):
            ...
            if asteroid.status != "failure":
                ...
                asteroid.error_msg = (
                    "Asteroid was processed but the output file was not created."
                )
            ...
            logger.warning(
                "Asteroid [ %s ] was processed but the output file was not created."
                % (asteroid.name)
            )
            ...
            try:
                ...
                update_create_astrometry_output(
                    asteroid,
                    "astrometry",
                    astrometry.get("filename"),
                    astrometry.get("file_size"),
                    astrometry.get("extension"),
                )
            except Exception as e:
                logger.warning("Failed to register Astrometry File. %s" % e)
            ...
            try:
                ...
                update_create_astrometry_output(
                    ...
                    "target_offset",
                    output.get("filename"),
                    output.get("file_size"),
                    output.get("extension"),
                )
            except Exception as e:
                logger.warning("Failed to register Target Offset. %s" % e)
            ...
            try:
                ...
                logger.warning("Failed to register Targets File. %s" % e)
            ...
            try:
                ...
                logger.warning("Failed to register PRAIA Target Search Log. %s" % e)
            ...
            try:
                ...
                logger.warning("Failed to register PRAIA Header Extraction. %s" % e)
            ...
            try:
                ...
                etime = results.get("header_extraction").get("execution_time", 0)
                asteroid.execution_header = timedelta(seconds=etime)
            except Exception as e:
                logger.warning(
                    "Failed to register execution time for Praia Header Extraction. %s"
                    % e
                )

            try:
                # Execution Time for Praia Astrometry
                etime = results.get("praia_astrometry").get("execution_time", 0)
                asteroid.execution_astrometry = timedelta(seconds=etime)
            except Exception as e:
                logger.warning(
            ...
            try:
                ...
                etime = results.get("praia_targets").get("execution_time", 0)
                asteroid.execution_targets = timedelta(seconds=etime)
            ...
            try:
                ...
                etime = results.get("plots").get("execution_time", 0)
                asteroid.execution_plots = timedelta(seconds=etime)

            except Exception as e:
                logger.warning("Failed to register execution times for Plots. %s" % e)

            if asteroid.status != "failure":
                if (
                    int(asteroid.processed_ccd_image) != int(asteroid.ccd_images)
                    and asteroid.status != "failure"
                ):
                    asteroid.status = "warning"
            ...
            asteroid.execution_time = (
                asteroid.execution_ccd_list
                + asteroid.execution_bsp_jpl
                + asteroid.execution_reference_catalog
                + asteroid.execution_header
                + asteroid.execution_astrometry
                + asteroid.execution_targets
                + asteroid.execution_plots
                + asteroid.execution_registry
            )
            ...
            logger.info(
                "Registered [ %s ] outputs for Astrometry Run [ %s ] Asteroid [ %s ] in %s"
        ...
        asteroid.error_msg = (
            "Asteroid was processed but failed to record results. Error: %s" % e
        )
        asteroid.save()
...
def finish_astrometry_run(astrometry_run):
    ...
    try:
        ...
        logger.info(
            "Astrometry Run successfully completed in %s"
    ...
    for astrometry_run in runs:
        ...
        if jobs.count() > 0:
            ...
            for job in jobs:
                ...
                logger.info(
                    "Check Status. Asteroid: [%s] Old Status: [%s] Current Status: [%s] ClusterId: [ %s ] "
                    % (
                        job.asteroid.name,
                        job.job_status,
                        result["JobStatus"],
...
def check_jobs_wait_to_run(astrometry_run):
    ...
    a = astrometry_run.asteroids.filter(status__in=["pending", "running", "idle"])
