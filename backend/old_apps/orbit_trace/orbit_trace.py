...
job.update({"status": "Running", "start": t0.isoformat()})
...
try:
    ...
    asteroids = retrieve_asteroids(job["filter_type"], job["filter_value"]).result()
    ...
    for asteroid in asteroids:
        ...
        asteroid.update({"bsp_jpl": bsp, "path": str(asteroid_path.absolute())})
    ...
    log.info(
        "Calculating theoretical positions Finished %s"
        % humanize.naturaldelta(step_tdelta, minimum_unit="microseconds")
    )
    ...
    log.info(
        "Calculating observed positions Finished %s"
        % humanize.naturaldelta(step_tdelta, minimum_unit="microseconds")
    )
    ...
    log.info(
        "Ingest the observations into the database Finished %s"
        % humanize.naturaldelta(step_tdelta, minimum_unit="microseconds")
    )
    ...
    log.info(
        "Write Asteroid Data in json Finish %s"
        % humanize.naturaldelta(step_tdelta, minimum_unit="microseconds")
    )
    ...
    job.update({"end": t1.isoformat(), "exec_time": tdelta.total_seconds()})
