...
from .models import (
    ...
    BspJplFile,
    ObservationFile,
    OrbitalParameterFile,
)
from .serializers import (
    OrbitRunSerializer,
    RefinedAsteroidSerializer,
    RefinedOrbitSerializer,
...
class OrbitRunViewSet(viewsets.ModelViewSet):
    ...
    def log_by_objects(self, request):
        ...
        try:
            ...
            obj_path = os.path.join(
                proccess.relative_path, "objects", name.replace(" ", "_")
            )
            ...
            if os.path.exists(log_file):
                ...
                return Response(dict({"success": False, "msg": "Deu Ruim"}))
                # else:
    ...
    def get_time_profile(self, request):
        ...
        try:
            ...
            asteroids = orbit_run.asteroids.exclude(status="failure").order_by(
                "start_time"
            )
...
class RefinedAsteroidViewSet(viewsets.ModelViewSet):
    ...
    def get_log(self, request):
        ...
        if id is not None:
            ...
            try:
                asteroid = RefinedAsteroid.objects.get(
                    name=str(name), orbit_run=int(run)
                )
            except ObjectDoesNotExist:
                return Response(
                    {
                        "success": False,
                        "msg": "Asteroid with name %s and Orbit Run %s Not Found."
                        % (name, run),
                    }
                )
        ...
        try:
            ...
            with open(log_path, "r") as fp:
                ...
                return Response(dict({"success": True, "lines": new_lines}))
    ...
    def get_neighbors(self, request):
        ...
        try:
            next_model = (
                RefinedAsteroid.objects.filter(orbit_run=asteroid.orbit_run)
