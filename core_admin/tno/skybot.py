    ...
    def start_import_skybot(self, run_id):
        ...
        try:
            ...
            if current_run.type_run == 'all':
                ...
                ImportSkybot(skybotrun=current_run).import_all_poitings()

            elif current_run.type_run == 'period':
                self.logger.info("Running Exposures with date in Period")

                ImportSkybot(skybotrun=current_run).import_poitings_by_period(
                ...
                ImportSkybot(skybotrun=current_run).import_pointings_by_radial_query(
                ...
                ImportSkybot(skybotrun=current_run).import_pointings_by_square_query(ra=current_run.ra_cent, dec=current_run.dec_cent,
...
def teste(file_path):
    LoadSkybot().read_skybot_output_csv(file_path)

    return "***** Sleeping: %s" % file_path
...
class ImportSkybot():
    ...
    def run_get_asteroids(self, pointings):
        ...
        with futures.ProcessPoolExecutor(max_workers=5) as ex:
            ...
            for pointing in pointings:
                ...
                file_path = self.get_asteroids_from_skybot(pointing)
                ...
                self.logger.debug("Download  %s/%s" %
                                  (current, len(pointings)))

                ex.submit(teste, file_path).add_done_callback(self.teste2)
                # self.logger.info(a)
        self.logger.info("***************** Done ****************")

    def teste2(self, future):
    ...
    def get_asteroids_from_skybot(self, pointing):
        ...
        self.statistics[file_id] = dict({
            ...
            'skybot_url': r.url,
            'download_start': t0.strftime("%Y-%m-%d %H:%M:%S"),
            'download_finish': t1.strftime("%Y-%m-%d %H:%M:%S"),
