...
class JhonstonArchive():
    ...
    def get_table_data(self):
        ...
        line_index = [x for x in range(len(string_table)) if string_table[x].startswith(
            'objects which should be designated as planets')]
        ...
        for ignore in ignore_lines:
            line_index = [x for x in range(
                len(string_table)) if string_table[x].startswith(ignore)]
        ...
        for line in string_table:
            if line.strip() is not '':
                ...
                diameter, diameter_flag = self.get_diameter(
                    line[99:108].strip())
    ...
    def write_csv(self, rows):
        ...
        with open(filename, 'w') as csvfile:
            fieldnames = ['number', 'name', 'provisional_designation', 'dynamical_class',
                          'a', 'e', 'perihelion_distance', 'aphelion_distance', 'i', 'diameter', 'diameter_flag', 'albedo', 'b_r_mag', 'taxon', 'density',
    ...
    def get_filename(self):
        ...
        filename = os.path.join(root_path, 'know_tnos_%s.csv' % today.strftime('%Y-%m-%d'))
