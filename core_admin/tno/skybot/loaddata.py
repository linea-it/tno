    ...
    def insert_or_update_asteroid(self, expnum, band, pointing, asteroid):
        ...
        try:
            stm_insert = skybot_output.insert().values(
                ...
                raj2000=ra,
                decj2000=dec,
                mv=float(getattr(asteroid, "mv", 0)),
                errpos=float(getattr(asteroid, "errpos", 0)),
                d=float(getattr(asteroid, "d", 0)),
                dracosdec=float(getattr(asteroid, "dra", 0)),
                ddec=float(getattr(asteroid, "ddec", 0)),
                dgeo=float(getattr(asteroid, "dg", 0)),
                dhelio=float(getattr(asteroid, "dh", 0)),
                phase=float(getattr(asteroid, "phase", 0)),
                solelong=float(getattr(asteroid, "sunelong", 0)),
                px=float(getattr(asteroid, "x", 0)),
                py=float(getattr(asteroid, "y", 0)),
                pz=float(getattr(asteroid, "z", 0)),
                vx=float(getattr(asteroid, "vx", 0)),
                vy=float(getattr(asteroid, "vy", 0)),
                vz=float(getattr(asteroid, "vz", 0)),
                jdref=float(getattr(asteroid, "epoch", 0)),
