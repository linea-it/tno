...
class PredictRunViewSet(viewsets.ModelViewSet):
    ...
    def get_time_profile(self, request):
        ...
        try:
            ...
            # Ephemeris Start + End
            start_ephemeris = (
                predict_run.asteroids.exclude(status__in=["failure", "not_executed"])
                .order_by("start_ephemeris")
                .first()
                .start_ephemeris
            )
            end_ephemeris = (
                predict_run.asteroids.exclude(status__in=["failure", "not_executed"])
                .order_by("finish_ephemeris")
                .last()
                .finish_ephemeris
            )
            ...
            start_catalog = (
                predict_run.asteroids.exclude(status__in=["failure", "not_executed"])
                .order_by("start_catalog")
                .first()
                .start_catalog
            )
            end_catalog = (
                predict_run.asteroids.exclude(status__in=["failure", "not_executed"])
                .order_by("finish_catalog")
                .last()
                .finish_catalog
            )
            ...
            start_search = (
                predict_run.asteroids.exclude(status__in=["failure", "not_executed"])
                .order_by("start_search_candidate")
                .first()
                .start_search_candidate
            )
            end_search = (
                predict_run.asteroids.exclude(status__in=["failure", "not_executed"])
                .order_by("finish_search_candidate")
                .last()
                    ...
                    "dates": dict(
                        {
                            ...
                            "start": datetime.strftime(
                                predict_run.start_time, "%Y-%m-%d %H:%M:%S"
                            ),
                    ...
                    "ephemeris": dict(
                        {
                            ...
                            "start": datetime.strftime(
                                start_ephemeris, "%Y-%m-%d %H:%M:%S"
                            ),
                            "finish": datetime.strftime(
                                end_ephemeris, "%Y-%m-%d %H:%M:%S"
                            ),
                            "rows": [],
                        }
                    ),
                    "catalog": dict(
                        {
                            ...
                            "start": datetime.strftime(
                                start_catalog, "%Y-%m-%d %H:%M:%S"
                            ),
                            "finish": datetime.strftime(
                                end_catalog, "%Y-%m-%d %H:%M:%S"
                            ),
                            "rows": [],
                        }
                    ),
                    "search": dict(
                        {
                            ...
                            "start": datetime.strftime(
                                start_search, "%Y-%m-%d %H:%M:%S"
                            ),
                            "finish": datetime.strftime(
                                end_search, "%Y-%m-%d %H:%M:%S"
                            ),
                            "rows": [],
                        }
                    ),
            ...
            for asteroid in asteroids:
                ...
                try:
                    if (
                        asteroid.status is not "not_executed"
                        and asteroid.status is not "failure"
                    ):
                        data["ephemeris"]["rows"].append(
                            dict(
                                {
                                    "id": asteroid.id,
                                    "name": asteroid.name,
                                    "start": datetime.strftime(
                                        asteroid.start_ephemeris, "%Y-%m-%d %H:%M:%S"
                                    ),
                                    "finish": datetime.strftime(
                                        asteroid.finish_ephemeris, "%Y-%m-%d %H:%M:%S"
                                    ),
                        ...
                        data["catalog"]["rows"].append(
                            dict(
                                {
                                    "id": asteroid.id,
                                    "name": asteroid.name,
                                    "start": datetime.strftime(
                                        asteroid.start_catalog, "%Y-%m-%d %H:%M:%S"
                                    ),
                                    "finish": datetime.strftime(
                                        asteroid.finish_catalog, "%Y-%m-%d %H:%M:%S"
                                    ),
                        ...
                        data["search"]["rows"].append(
                            dict(
                                {
                                    "id": asteroid.id,
                                    "name": asteroid.name,
                                    "start": datetime.strftime(
                                        asteroid.start_search_candidate,
                                        "%Y-%m-%d %H:%M:%S",
                                    ),
                                    "finish": datetime.strftime(
                                        asteroid.finish_search_candidate,
                                        "%Y-%m-%d %H:%M:%S",
                                    ),
                        ...
                        data["map"]["rows"].append(
                            dict(
                                {
                                    "id": asteroid.id,
                                    "name": asteroid.name,
                                    "start": datetime.strftime(
                                        asteroid.start_maps, "%Y-%m-%d %H:%M:%S"
                                    ),
                                    "finish": datetime.strftime(
                                        asteroid.finish_maps, "%Y-%m-%d %H:%M:%S"
                                    ),
...
class PredictAsteroidViewSet(viewsets.ModelViewSet):
    ...
    def catalog_positions(self, request):
        ...
        try:
            ...
            return Response({"success": True, "results": data, "count": len(data)})
    ...
    def get_neighbors(self, request):
        ...
        try:
            next_model = (
                PredictAsteroid.objects.filter(predict_run=asteroid.predict_run)
                .exclude(status__in=["failure", "not_executed"])
                .filter(id__gt=asteroid.id)
                .order_by("id")
                .first()
            )
    ...
    def download_results(self, request):
        ...
        if id is not None:
            ...
            try:
                ...
                return Response(
                    {
                        "success": False,
                        "msg": "Asteroid with name %s and Predict Run %s Not Found."
                        % (name, run),
                    }
                )
        ...
        if not os.path.exists(zip_file):
            return Response(
                dict({"success": False, "msg": "Failed to create zip file"})
            )
...
class OccultationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        most_recent_only = bool(
            self.request.query_params.get("most_recent_only", False)
        )

        if most_recent_only:
            queryset = (
                Occultation.objects.select_related("asteroid")
                .select_related("asteroid__predict_run")
                .distinct("asteroid__name", "date_time")
                .filter(asteroid__predict_run__status="success")
                .order_by(
