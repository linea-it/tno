    ...
    <Card>
      ...
      <CardContent>
        ...
        <Typography variant='body1' component='span'>
          ...
          <br />
          <ul>
            <li>Os jobs estão divididos em blocos de aproximadamente ~5000 exposures.</li>
            <li>Duração estimada de cada job é de ~3h</li>
            <li>Algumas informações de estimativa do job só ficaram disponiveis quando o job for iniciado.</li>
          </ul>
        ...
        <FormGroup>
          <FormControlLabel control={<Switch checked={checked} onChange={handleChange} />} label='Li e entendi.' />
        </FormGroup>
        <LoadingButton size='large' disabled={!checked} onClick={handleClick} loading={status === 'loading'} variant='contained'>
          SUBMIT SKYBOT JOBS
        </LoadingButton>
        {open === true && (
          <>
            {status === 'success' && (
              ...
              >
                Foram gerados {data.data.count} jobs.
              </Alert>
            )}
            {status === 'error' && (
              <Alert severity='error'>Não foi possivel executar a função "/api/des/skybot_job/submit_job_balanced_periods/".</Alert>
            )}
          </>
        )}
      </CardContent>
