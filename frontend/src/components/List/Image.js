    ...
    <>
      <Grid container spacing={2}>
        {data.map((row, i) => {
          ...
          const asteroidName = parseInt(row.asteroid_number) > 0 ? `${row.asteroid_name} (${row.asteroid_number})` : row.asteroid_name
          ...
          return (
            <Grid key={i} item xs={12} xl={4} lg={6} md={4}>
              <Card>
                ...
                <CardContent>
                  <img src={baseUrl + row.src} className={classes.occMapImg} alt={asteroidName} onClick={() => handleImageClick(row.id)} />
                </CardContent>
              </Card>
            </Grid>
          )
