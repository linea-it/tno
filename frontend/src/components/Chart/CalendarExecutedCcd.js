...
function CalendarExecutedCcd({ data }) {
  const [rows, setRows] = useState([]);

  useEffect(() => {
    const ccds = data.map((row) => {
      let value = 0;

      if (row.count > 0 && row.downloaded === row.count) {
        value = 2;
      } else if (row.count > 0 && row.downloaded !== row.count) {
        value = 1;
      }
  ...
  useEffect(() => {
    drawCalendar(rows);
    const svg = document.getElementById('executed-ccd-svg');
  ...
  return (
    <div>
      <svg id="executed-ccd-svg" />
    </div>
  );
}

CalendarExecutedCcd.propTypes = {
  data: PropTypes.arrayOf(PropTypes.object).isRequired,
};

export default CalendarExecutedCcd;
