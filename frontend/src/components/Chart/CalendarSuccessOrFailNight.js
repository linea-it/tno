...
function CalendarSuccessOrFailNight({ data }) {
  ...
  function drawCalendar(dateValues) {
    ...
    const colorFn = d3.scaleLinear().domain([0, 1, 2, 3]).range(['#ebedf0', '#939497', '#212121', '#FF3333'])
  ...
  useEffect(() => {
    drawCalendar(rows)
    const svg = document.getElementById('success-fail-night-svg')
  ...
  return (
    <div>
      <svg id='success-fail-night-svg' />
    </div>
  )
}

CalendarSuccessOrFailNight.propTypes = {
  data: PropTypes.arrayOf(PropTypes.object).isRequired
}

export default CalendarSuccessOrFailNight
