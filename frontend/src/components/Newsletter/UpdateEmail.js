...
export default function UpdateEmail(subscriptionId) {
  ...
  // TODO: Revisar o componente de atualização de email.
  useEffect(() => {
    // getEmailDb()
    // .then((res) => {
    //   setResultsEmail(res)
  ...
  return (
    <Box autoComplete='off' noValidate sx={{ mt: 1, display: 'block' }}>
      ...
      <Dialog component='form' onSubmit={handleSubmit} open={openDialog} onClose={handleClose}>
        ...
        <DialogContent>
          <DialogContentText>Please enter your new email address here.</DialogContentText>
          <TextField
            margin='normal'
            required
            fullWidth
            ...
            autoComplete='off'
            autoFocus
            sx={{ input: { borderRadius: '6px', height: '8px', color: 'gray' } }}
            value={email}
        ...
        <DialogActions>
          ...
          >
            <Alert onClose={handleCloseSnackbar} severity={open === 'success' ? 'success' : 'warning'}>
              <AlertTitle>{open === 'success' ? 'Success' : 'Warning'}</AlertTitle>
              <p>
                {open === 'success'
